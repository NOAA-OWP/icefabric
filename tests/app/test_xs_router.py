...
def test_xs_endpoint(remote_client):
    ...
    for mip_hucid in mip_hucid_list[:5]:
        response = remote_client.get(f"/v1/ras_xs/{mip_hucid}?xstype=mip")
        ...
        response2 = remote_client.get(f"/v1/ras_xs/{mip_hucid}/dsreachid={reachid}?xstype=mip")
        assert response2.status_code == 200, "Incorrect response"

    for ble_hucid in ble_hucid_list[:5]:
        response = remote_client.get(f"/v1/ras_xs/{ble_hucid}?xstype=ble")
        assert response.status_code == 200, "Incorrect response"

        reachid = list(set(catalog.load_table(f"ble_xs.{ble_hucid}").scan().to_pandas()["ds_reach_id"]))[0]
        response2 = remote_client.get(f"/v1/ras_xs/{ble_hucid}/dsreachid={reachid}?xstype=ble")
        assert response2.status_code == 200, "Incorrect response"


def test_xs_subset_endpoint__422(remote_client):
    """Test: GET /v1/ras_xs/08020203?xstype=NA - fails validator"""
    response = remote_client.get("/v1/ras_xs/08020203?xstype=NA")
    assert response.status_code == 422

    response2 = remote_client.get("/v1/ras_xs/08020203/dsreachid=NA?xstype=NA")
